...
- 구현 내용과 기능 설명
    - 1번 사진(메시지 몇개 넣은 처음 사진, 시간 및 현대 대화 수 구현), 2번 사진(빈 입력 차단 기능 구현 사진)
...
![과제4 실행화면](img/work4Screen4.png)
  ...
  - 구현 내용과 기능 설명
    - 1번 사진(메시지 몇개 넣은 처음 사진), 2번 사진(메시지 선택삭제 구현 사진), 3번 사진(전체 삭제 기능 사진), 4번 사진(50자 이상 입력시 경고 및 제한 기능 사진)
...
## 배운 내용
 - 이전 주차보다 더 많은 내용(문자열, 현재 시각 입력 등)을 구현해야해서 힘들었지만, 그만큼 유익했다.
 - 특히 텍스트는 해당 객체의 text필드로 저장된다는 것을 깨달았다.
 - 버그로 인해 문제가 발생했음에도 원인을 찾지 못했을 때는, 생성형 인공지능의 도움을 받아 해결했다. 